...
def get_layout_predictions(layout_session, label_map_layout, ca_img, layout_output, f_idx, p, input_name, backend='yolo'):
    ...
    if backend == 'yolo':
        layout_predictions = \
            non_max_suppression(layout_predictions, conf_thres=0.05, iou_thres=0.01, max_det=300, agnostic=True)[0]
        print(layout_predictions.size())
    elif backend == 'yolov8':
        layout_predictions = \
            non_max_supression_yolov8(layout_predictions, conf_thres=0.01, iou_thres=0.1, max_det=2000, agnostic=True)[
                0]

    layout_bboxes, layout_probs, layout_labels = layout_predictions[:, :4], layout_predictions[:,
                                                                            -2], layout_predictions[:, -1]
    ...
    for i, (line_bbox, pred_class) in enumerate(zip(layout_bboxes, layout_labels)):
        ...
        x0, y0, x1, y1 = int(floor((x0.item() - w_trans) * im_width / w_ratio)), int(
            floor((y0.item() - h_trans) * im_height / h_ratio)), \
                         int(ceil((x1.item() - w_trans) * im_width / w_ratio)), int(
            ceil((y1.item() - h_trans) * im_height / h_ratio))
        ...
        if label_map_layout[int(pred_class.item())] in LAYOUT_TYPES_TO_EFFOCR:
            crops = get_crops_from_layout_image(
                layout_crop)  # Chunk the crop if it's a poor aspect ratio for line detection
            for crop in crops:
                crops_for_ocr.append((i, crop))

        if layout_output:  # optionally save layout bounding boxes to file
            draw.rectangle((x0, y0, x1, y1),
                           outline=LAYOUT_COLOR_MAP.get(label_map_layout[int(pred_class.item())], 'black'), width=5)
    ...
    if layout_output:  # Save the image with drawn bounding boxes if requested
        layout_img.save(os.path.join(layout_output, f'layout_boxes_{f_idx}_p{p}.jpg'))

    return crops_for_ocr, layout_crops
...
def get_line_predictions(line_session, input_name, crops_for_ocr, backend='yolo'):
    bbox_preds = {}
    for bbox_idx, line_bbox in crops_for_ocr:
        gc.collect()
        ...
        if backend == 'yolo':
            ...
            line_predictions = non_max_supression_yolov8(line_predictions, conf_thres=0.2, iou_thres=0.15, max_det=200)[
                0]
...
def spell_check_results(results, delimiter=None):
    ...
    for k in results.keys():
        try:
            if delimiter == ' ':
                results[k] = ' '.join(
                    [sym_spell.lookup(word, Verbosity.CLOSEST, max_edit_distance=2, include_unknown=True)[0].term for
                     word in results[k].split()])
            elif delimiter:
                results[k] = delimiter.join(
                    [sym_spell.lookup_compound(word, max_edit_distance=2, transfer_casing=True)[0].term for word in
                     results[k].split(delimiter)])
            else:
...
def googleocr(
        ...
        file_path: str = '../AmericanStories/temp/',
) -> None:
    ...
    crop.save(os.path.join(file_path, f'layout_box_{i}_{f_idx}_p{p}.png'))
    with open(os.path.join(file_path, f'layout_box_{i}_{f_idx}_p{p}.png'), "rb") as image:
...
def main(args):
    ...
    # file_path = "C:/Users/Perry Carter/Dropbox (Princeton)/Dissertation/Narrative Analysis/Images/full/000b2676593b0cd1d24863f052950e1f9d4a91b7.pdf"
    # processor_display_name = "new_processor"  # Must be unique per project, e.g.: "My Processor"
    # Google doc AI args
    ...
    for f_idx, f in enumerate(filenames):
        ...
        try:
            if os.path.isfile(f):
                if f.endswith(IMG_FILE_EXS):
                    ca_imgs = cv2.imread(f, cv2.IMREAD_COLOR)
                elif f.endswith('.pdf'):
                    pdfs_to_images(
                        source_path=f,
                    ...
                    for i in os.listdir(manifest_path):
                        if os.path.isfile(os.path.join(manifest_path, i)) and f[9:-4] in i and ".pdf" not in i:
                            ca_imgs.append(cv2.imread(os.path.join(manifest_path, i), cv2.IMREAD_COLOR))

                else:
                    print('Unknown file type for {}, only jpg, png, jp2, pdf supported!'.format(f))
                    continue
                    # Not using CA - not needed
            # else:
            ...
            #
            #     if len(str(pg_num)) == 1:
            #         pg_num_str = '0' + str(pg_num)
            #     else:
            #         pg_num_str = str(pg_num)
            #     #CA Specific
            #     # date = ed[:8]
        ...
        logging.info('Image Load Time: {}'.format(image_download_time))
        for p, ca_img in enumerate(ca_imgs):
            if ca_img is None:
            ...
            try:
                crops_for_ocr, layout_crops = get_layout_predictions(layout_inf_sess, label_map_layout, ca_img,
                                                                     layout_output,
                                                                     f_idx, p, layout_input_name,
                                                                     backend=layout_model_backend)
            except Exception as e:
            ...
            legible_crops = []
            leg_dict = {}
            print('Num crops to effocr: ', len(crops_for_ocr))
            illegible_crops = []
            try:
                for i, crop in enumerate(crops_for_ocr):
                    leg_preds = legibility_classifier.run(None, {
                        leg_input_name: legibility_transform(crop[1]).unsqueeze(0).numpy()})
            ...
            #         line_crops = []
            #         try:
            #             bbox_line_preds = get_line_predictions(line_inf_sess, line_input_name, crops_for_ocr,
            #                                                    backend=line_model_backend)
            #
            ...
            #
            inference_results = []
            for i, crop in crops_for_ocr:
                inference_results.append(googleocr(project_id, processor_id, crop, i, f_idx, p))
            files = os.listdir('../AmericanStories/temp/')
            for file in files:
                os.chmod('../AmericanStories/temp/' + file, 0o777)
                os.remove('../AmericanStories/temp/' + file)
            metadata['bboxes'] = []
            ...
            if os.path.isdir(manifest_path):
                with open(os.path.join(output_save_path,
                                       os.path.basename(filenames[f_idx]).strip()[:-4]+f'_p{p}' + '.json'),
                          'w') as file:
                    json.dump(metadata, file, indent=2)
            else:
                with open(os.path.join(output_save_path,
                                       "{}.json".format('_'.join((filenames[f_idx].strip()[:-4]+f'_p{p}').split('/')[-4:]))),
                          "w") as file:
                    json.dump(metadata, file, indent=2)

            # if bbox_output:
            #     with open(os.path.join(output_save_path,
            #                            "inference_coco_{}.json".format(filenames[f_idx].strip().split('/')[-1][:-4])),
            #               "w") as f:
            #         # TODO: include Google OCR stuff here
...
if __name__ == '__main__':
    ...
    parser.add_argument("--location",
                        help="Location either eu or us")
    parser.add_argument("--processor_id",
                        help="unique id for google document AI processor")
